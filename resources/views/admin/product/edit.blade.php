                ...
                <div class="section" id="detail-page">
                    <div class="section-detail">
                        ...
                        <form method="POST" enctype="multipart/form-data">
                                    ...
                                    @foreach ($Medias as $media)
                                        @if ($product->id == $media->product_id)
                                            <div class="image_show" data-path="{{ $media->thumb }}">
                                                <div class="checkbox-container"><input type="checkbox" name="delete_image"
                                                        value="{{ $media->thumb }}"></div>
                                                <a href="" target="blank">
                                                    <img src="{{ $media->thumb }}" width="100px" id="urlImage">
                                                </a>
                                                {{-- <button type="button" class="btn btn-info btn-lg show_modal" data-toggle="modal" data-path ="{{$media->thumb}}"
                                                    data-target="#myModal" data-id ="{{ $media->id }}">Sửa</button> --}}
                                            </div>
                                        @endif
                                    @endforeach
                                </div>
                            ...
                            <button type="button"
                                name="btn-delete"url-delete="{{ route('Upload.delete') }}"id="deleteImage">Xóa ảnh</button>
                            ...
                            <select name="cat_id">
                                ...
                                @foreach ($productCats as $item)
                                    <option
                                        value="{{ $item->id }}"{{ $product->cat_id == $item->id ? 'selected' : '' }}>
                                        {{ $item->name }}</option>
                                @endforeach
                            </select>
